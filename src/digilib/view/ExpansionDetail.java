...
public class ExpansionDetail extends View {
	...
	public ExpansionDetail() {
		this.TableName = "EXPANSIONDETAILVIEW";
	}
	...
	public static boolean add(Long newReferenceNo,
		...
		CallableStatement cstm = conn.prepareCall("{?=call EXPANSIONS.ADD(?,?)}");
	...
	public static boolean remove(Long newDocumentNo,
		...
		boolean result = false;
		Connection conn = DBConnectionManager.getInstance().getConnection();
		CallableStatement cstm = conn.prepareCall("{?=call EXPANSIONS.REMOVE(?,?,?)}");
		cstm.registerOutParameter(1, Types.NUMERIC);
