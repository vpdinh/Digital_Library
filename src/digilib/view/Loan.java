...
public class Loan extends View {
	...
	public static boolean post(Long newDocumentNo,
		...
		CallableStatement cstm = conn.prepareCall("{?=call LOANS.POST(?,?)}");
	...
	public LoanDetail getDetails() throws SQLException {
		LoanDetail loanDetails = new LoanDetail();
		loanDetails.setCriteria("DOCUMENTNO = " + this.getDocumentNo().longValue());
		return loanDetails;
	}
}