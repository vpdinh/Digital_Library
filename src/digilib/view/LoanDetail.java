...
public class LoanDetail extends View {
	...
	public static boolean add(Long newReferenceNo,
		...
		CallableStatement cstm = conn.prepareCall("{?=call LOANS.ADD(?,?)}");
	...
	public static boolean remove(Long newDocumentNo,
		...
		boolean result = false;
		Connection conn = DBConnectionManager.getInstance().getConnection();
		CallableStatement cstm = conn.prepareCall("{?=call LOANS.REMOVE(?,?,?)}");
		cstm.registerOutParameter(1, Types.NUMERIC);
