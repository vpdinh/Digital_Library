...
import digilib.view.Librarian;


public class _librarians extends oracle.jsp.runtime.HttpJsp {
    ...
    try {
      ...
      out.print(__jsp_StaticText.text[4]);
      	...
      	if (sAction.equalsIgnoreCase("login")) {
      		...
      		action = 1;
      	} else if (sAction.equalsIgnoreCase("add")) {
      		action = 10;
      	...
      	if ((action != 0 && !curLibrarian.isValidated()) || !request.getMethod().equalsIgnoreCase("post")) {
      		...
      		switch (action) {
      			case 0 : //Login
      				try {
      					sLibrarianID = request.getParameter("LibrarianID");
      					sPassword = request.getParameter("Password");
      					src = request.getParameter("src");
      					if (curLibrarian.validate(sLibrarianID, sPassword)) {
      						sAnnouncement = "Xác nhận thủ thư hợp lệ !!!";
      					} else {
      						sAnnouncement = "Xác nhận thủ thư không hợp lệ !!!";
      					}
      				} catch (Exception e) {
      			...
      			case 10 : //add
      				try {
      					...
      					sPassword = request.getParameterValues("Password")[affected];
      					sStatus = request.getParameterValues("Status")[affected];
      					src = request.getHeader("Referer");
      					if (Librarian.add(sLibrarianID, sName, sPassword, new Integer(sStatus), curLibrarian.getLibrarianID())) {
      						sAnnouncement = "Thêm Thủ thư thành công !!!";
      					} else {
      						sAnnouncement = "Thêm Thủ thư không thành công !!!";
      					}
      				} catch (Exception e) {
      			...
      			case 11 : //update
      				try {
      					...
      					sStatus = request.getParameterValues("Status")[affected];
      					src = request.getHeader("Referer");
      					if (Librarian.update(sLibrarianID, sName, sPassword, new Integer(sStatus), curLibrarian.getLibrarianID())) {
      						sAnnouncement = "Cập nhật dữ liệu Thủ thư thành công !!!";
      						if (sLibrarianID.equalsIgnoreCase(curLibrarian.getLibrarianID())) {
      							curLibrarian.validate(sLibrarianID, sPassword);
      						}
      					} else {
      						sAnnouncement = "Cập nhật dữ liệu Thủ thư không thành công !!!";
      					}
      				} catch (Exception e) {
      					sAnnouncement = "Hệ thống hiện tại không xử lý được<br />xin vui lòng thử lại sau";
      				}
