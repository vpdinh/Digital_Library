...
<title>D I G I L I B &gt; Xử lý &gt; Định dạng</title>
<%@ page import="digilib.view.Language" %>
		...
		switch (action) {
			case 10 : //add
				try {
					...
					sLanguageID = request.getParameterValues("LanguageID")[affected];
					sName = request.getParameterValues("Name")[affected];
					src = request.getHeader("Referer");
					if (Language.add(sLanguageID, sName, curLibrarian.getLibrarianID())) {
						sAnnouncement = "Thêm Định dạng thành công !!!";
					} else {
						sAnnouncement = "Thêm Định dạng không thành công !!!";
					}
				} catch (Exception e) {
			...
			case 11 : //update
				try {
					...
					sName = request.getParameterValues("Name")[affected];
					src = request.getHeader("Referer");
					if (Language.update(sLanguageID, sName, curLibrarian.getLibrarianID())) {
			...
			case 12 : //remove
				try {
					...
					sLanguageID = request.getParameterValues("LanguageID")[affected];
					src = request.getHeader("Referer");
					if (Language.remove(sLanguageID, curLibrarian.getLibrarianID())) {
