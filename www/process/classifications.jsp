...
<title>D I G I L I B &gt; Xử lý &gt; Thang phân loại Dewey</title>
<%@ page import="digilib.view.Classification" %>
		...
		switch (action) {
			case 10 : //add
				try {
					...
					sDDCID = request.getParameterValues("DDCID")[affected];
					sName = request.getParameterValues("Name")[affected];
					src = request.getHeader("Referer");
					if (Classification.add(sDDCID, sName, curLibrarian.getLibrarianID())) {
						sAnnouncement = "Thêm phân loại thành công !!!";
					} else {
						sAnnouncement = "Thêm phân loại không thành công !!!";
					}
				} catch (Exception e) {
			...
			case 11 : //update
				try {
					...
					sName = request.getParameterValues("Name")[affected];
					src = request.getHeader("Referer");
					if (Classification.update(sDDCID, sName, curLibrarian.getLibrarianID())) {
						sAnnouncement = "Cập nhật dữ liệu phân loại thành công !!!";
					} else {
						sAnnouncement = "Cập nhật dữ liệu phân loại không thành công !!!";
					}
				} catch (Exception e) {
					sAnnouncement = "Hệ thống hiện tại không xử lý được<br />xin vui lòng thử lại sau";
				}
			...
			case 12 : //remove
				try {
					...
					sDDCID = request.getParameterValues("DDCID")[affected];
					src = request.getHeader("Referer");
					if (Classification.remove(sDDCID, curLibrarian.getLibrarianID())) {
						sAnnouncement = "Xoá phân loại thành công !!!";
					} else {
						sAnnouncement = "Xoá phân loại không thành công !!!";
					}
				} catch (Exception e) {
