...
<%@ page errorPage="/errorshandler.jsp" %>
<%
...
<title>D I G I L I B &gt; Xử lý &gt; Tài liệu</title>
		...
		switch (action) {
			case 10 : //add
				try {
					...
					sItemNo = request.getParameterValues("ItemNo")[affected];
					sCopyType = request.getParameterValues("CopyType")[affected];
					sLocation = request.getParameterValues("Location")[affected];
					src = request.getHeader("Referer");
					if (Copy.add(new Long(sItemNo), new Integer(sCopyType), sLocation, new Integer(1), curLibrarian.getLibrarianID())) {
						sAnnouncement = "Thêm phiên bản thành công !!!";
					} else {
						sAnnouncement = "Thêm phiên bản không thành công !!!";
					}
				} catch (Exception e) {
			...
			case 11 : //update
				try {
					...
					sLocation = request.getParameterValues("Location")[affected];
					src = request.getHeader("Referer");
					if (Copy.update(sItemNo,  sName, curLibrarian.getLibrarianID())) {
						sAnnouncement = "Cập nhật dữ liệu Định dạng thành công !!!";
					} else {
						sAnnouncement = "Cập nhật dữ liệu Định dạng không thành công !!!";
					}
				} catch (Exception e) {
					sAnnouncement = "Hệ thống hiện tại không xử lý được<br />xin vui lòng thử lại sau";
				}
			...
			case 12 : //remove
				try {
					sAffected = request.getParameter("affected");
					affected = Integer.parseInt(sAffected);
					sFormatID = request.getParameterValues("FormatID")[affected];
					src = request.getHeader("Referer");
					if (Format.remove(sFormatID, curLibrarian.getLibrarianID())) {
						sAnnouncement = "Xoá Định dạng thành công !!!";
					} else {
						sAnnouncement = "Xoá Định dạng không thành công !!!";
					}
				} catch (Exception e) {
					sAnnouncement = "Không thể xoá Định dạng<br />xin vui lòng thử lại sau";
				}
				break;
				*/
		}
