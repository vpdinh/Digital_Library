...
<title>D I G I L I B &gt; Xử lý &gt; Xử lý cuối ngày</title>
<%@ page import="digilib.view.DayEnd" %>
...
<%
	...
	String src = "/librarians/";
	try {
		sAction = request.getParameter("action");
	} catch (Exception e) {}
	if (sAction.equalsIgnoreCase("processbookingoverdue")) {
		action = 20;
	} else if (sAction.equalsIgnoreCase("processloanoverdue")) {
		action = 21;
	}
	if (!curLibrarian.isValidated()) {
		...
		switch (action) {
			case 20 : //process Booking Overdue
				try {
					if (DayEnd.processBookingOverdue(curLibrarian.getLibrarianID())) {
						sAnnouncement = "Xử lý thành công yêu cầu !!!";
					} else {
						sAnnouncement = "Xử lý không thành công yêu cầu !!!";
					}
				} catch (Exception e) {
					sAnnouncement = "Hệ thống hiện tại không xử lý được<br />xin vui lòng thử lại sau<br />" + e.toString();
				}
				src = "/librarians/management/dayend.jsp";
				break;
			case 21 : //post
				try {
					if (DayEnd.processLoanOverdue(curLibrarian.getLibrarianID())) {
						sAnnouncement = "Xử lý thành công yêu cầu !!!";
					} else {
						sAnnouncement = "Xử lý không thành công yêu cầu !!!";
					}
