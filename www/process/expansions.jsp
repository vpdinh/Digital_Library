...
<%@ page import="digilib.view.Expansion" %>
		...
		switch (action) {
			case 10 : //add
				try {
					...
					for (int i = 0; i < totalRequest; i++) {
						//try {
							if (ExpansionDetail.add(new Long(sTransactionNo[i]), curMember.getMemberID())) {
								processSuccessed++;
							}
						//} catch (Exception e2) {}
					}
			...
			case 11 : //post
				try {
					...
					for (int i = 0; i < totalRequest; i++) {
						try {
							if (Expansion.post(new Long(sDocumentNo[i]), curMember.getMemberID())) {
								processSuccessed++;
							}
						} catch (Exception e2) {}
					}
			...
			case 12 : //remove
				try {
					...
					for (int i = 0; i < totalRequest; i++) {
						try {
							if (ExpansionDetail.remove(new Long(sDocumentNo[i]), new Long(sItemNo[i]), curMember.getMemberID())) {
								processSuccessed++;
							}
						} catch (Exception e2) {}
					}
