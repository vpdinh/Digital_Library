...
<title>D I G I L I B &gt; Xử lý &gt; Cho mượn tài liệu</title>
<%@ page import="digilib.view.Loan" %>
...
<%
	...
	String[] sItemNo = null;
	String[] sTransactionNo = null;
	String src = null;
	int totalRequest = 0;
	int processSuccessed = 0;
	if (!curLibrarian.isValidated() || !request.getMethod().equalsIgnoreCase("post")) {
		...
		switch (action) {
			case 10 : //add
				try {
					sTransactionNo = request.getParameterValues("accepted");
					totalRequest = sTransactionNo.length;
					for (int i = 0; i < totalRequest; i++) {
						try {
							if (LoanDetail.add(new Long(sTransactionNo[i]), curLibrarian.getLibrarianID())) {
								processSuccessed++;
							}
						} catch (Exception e2) {}
					}
				...
				src = "/librarians/management/confirmloans.jsp";
				break;
			case 11 : //post
				try {
					...
					for (int i = 0; i < totalRequest; i++) {
						try {
							if (Loan.post(new Long(sDocumentNo[i]), curLibrarian.getLibrarianID())) {
								processSuccessed++;
							}
						} catch (Exception e2) {}
					}
				...
				src = "/librarians/management/loans.jsp";
				break;
			case 12 : //remove
				try {
					...
					for (int i = 0; i < totalRequest; i++) {
						try {
							if (LoanDetail.remove(new Long(sDocumentNo[i]), new Long(sItemNo[i]), curLibrarian.getLibrarianID())) {
								processSuccessed++;
							}
						} catch (Exception e2) {}
					}
